...
A recurrence relation is a way of defining the terms of a sequence with
 respect to the values of previous terms. In the case of Fibonacci's
  ...
  rabbits that were alive the previous month, plus any new offspring.
  ...
  As a result, if Fn
represents the number of rabbit pairs alive after the n-th month, t
hen we obtain the Fibonacci sequence having terms Fn that are defined
...
relation, we can simply use the recurrence relation to generate
terms for progressively larger values of n. This problem introduces
us to the computational technique of dynamic programming, which
...
of reproduction-age rabbits produces a litter of k rabbit pairs (instead
 of only 1 pair).'''


def growth(n,k): # n is months, k is litter size
    if n == 0:
        return 0
    elif n == 1:
        return 1
    else:
        return growth(n-1,k) + k*growth(n-2,k)
print(growth(29,3))
